...
impl Processor for BallOccurProcessor {
    ...
    async fn execute(&mut self, context: &mut Context) -> Result<(), Error> {
        ...
        Self::prepare_occurs(&mut blue_balls_occur_indexes, &mut blue_balls_occurs);
        Self::prepare_occurs(&mut red_balls_occur_indexes, &mut red_balls_occurs);

        let prized_blue_balls = blue_balls_occurs.keys().copied().collect::<Vec<usize>>();
        Self::prepare_prized_balls(&mut blue_balls_occurs, prized_blue_balls);
        let prized_red_balls = red_balls_occurs.keys().copied().collect::<Vec<usize>>();
        Self::prepare_prized_balls(&mut red_balls_occurs, prized_red_balls);
...
impl BallOccurProcessor {
    fn prepare_occurs(
        balls_occur_indexes: &mut HashMap<usize, Vec<usize>>,
        balls_occurs: &mut HashMap<usize, BallOccurInfo>,
    ) {
        balls_occur_indexes
            .iter()
    ...
    fn prepare_prized_balls(
        balls_occurs: &mut HashMap<usize, BallOccurInfo>,
        prized_balls: Vec<usize>,
    ) {
        prized_balls.into_iter().for_each(|blue_ball| {
            balls_occurs.entry(blue_ball).and_modify(|occur_info| {
